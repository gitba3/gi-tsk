...
var WebCodeCamJS = function(element) {
  ...
  function gotSources(device) {
    if (device.kind === "video" || device.kind === "videoinput") {
      ...
      var text =
        device.label ||
        "camera " + (videoSelect.length + 1) + " (facing: " + face + ")";
      html(
        '<option value="' +
  ...
  return {
    init: function(opt) {
      ...
      var workerData = new Blob(
        [document.getElementById("decoderWorker").textContent],
        {
          type: "text/javascript"
        }
      );
      DecodeWorker = new Worker(window.URL.createObjectURL(workerData));
      if (options.decodeQRCodeRate || options.decodeBarCodeRate) {
        setCallBack();
      }
      return this;
